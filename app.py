...
st.markdown('This app uses tweepy to get tweets from twitter based on the input name/phrase. It then processes the '
            'tweets through HuggingFace transformers pipeline function for sentiment analysis. The resulting sentiments'
            ' and corresponding tweets are then put in a dataframe for display which is what you see as result.')
...
def run():
    with st.form(key='Enter name'):
        ...
        number_of_tweets = st.number_input('Enter the number of latest tweets for which you want to know the sentiment'
                                           '(Maximum 50 tweets)', 0, 50, 10)
        submit_button = st.form_submit_button(label='Submit')
        if submit_button:
            tweets = tweepy.Cursor(api.search_tweets, q=search_words, lang="en").items(number_of_tweets)
